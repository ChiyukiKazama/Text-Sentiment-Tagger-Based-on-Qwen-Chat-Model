...
def load_dataset(dataset_size, csv_path="./dataset/ChnSentiCorp.csv"):
    ...
    try:
        ...
        # random_state=None 确保每次运行的结果可复现，如需完全随机可删除此参数
        # ----------------
...
if __name__ == "__main__":
    ...
    for i, (text, true_label) in enumerate(zip(texts, true_labels)):
        ...
        print(
            f"[{i + 1:3d}/{len(texts):3d}] {status} 真实: {true_label:2s} | 预测: {predicted_label:2s} | 文本: {text[:50]}...")

        total_predictions += 1

        # 将每一条预测结果和原文本添加到列表中
        results_data.append({'Sentiment': predicted_label, 'Comment': text})
    ...
    if results_data:
        ...
        output_filename = f"./result/results_{int(time.time())}.csv"
